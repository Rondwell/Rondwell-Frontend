<script lang="ts">
	...
	export let iconColor: string = '#0D151C';
	export let iconBgColor: string = 'white';
</script>

<div class="flex items-center justify-between">
   <div class="flex items-center gap-[9px] justify-center">
     <img src={organizerAvatar} alt={`${organizerName}-avatar`} class="size-[27px] rounded-full outline outline-[#0D151C]/8 object-cover" on:error={(e) => { (e.currentTarget as HTMLImageElement).src = '/you-rondwell.png'; }} />
     <h2 class="leading-6 text-base font-normal" style="color: {iconColor};">{organizerName}</h2>
   </div>
   {#if organizerSocial}
   <div class="flex items-center justify-center gap-0.5">
    ...
    <a href="https://instagram.com/{socialLinks.instagram}" target="_blank" rel="noopener" aria-label="Instagram" class="size-7.5 inline-flex justify-center items-center opacity-50 hover:opacity-100 transition-opacity">
        <svg width="18" height="18" viewBox="0 0 18 18" fill="none"><path fill-rule="evenodd" clip-rule="evenodd" d="M0 8.64C0 5.616 0 4.104.588 2.948A4.5 4.5 0 012.949.589C4.104 0 5.614 0 8.64 0h.72c3.024 0 4.536 0 5.693.588a4.5 4.5 0 012.359 2.36C18 4.104 18 5.614 18 8.64v.72c0 3.024 0 4.536-.588 5.693a4.5 4.5 0 01-2.36 2.359C13.896 18 12.386 18 9.36 18h-.72c-3.024 0-4.536 0-5.693-.588A4.5 4.5 0 01.589 15.052C0 13.896 0 12.386 0 9.36V8.64z" fill="{iconColor}" fill-opacity=".36"/><path fill-rule="evenodd" clip-rule="evenodd" d="M9 13.5a4.5 4.5 0 100-9 4.5 4.5 0 000 9zm0-1.8a2.7 2.7 0 100-5.4 2.7 2.7 0 000 5.4z" fill="{iconBgColor}"/><circle cx="13.7" cy="4.27" r="1.35" fill="{iconBgColor}"/></svg>
    </a>
    {/if}
    {#if socialLinks.x}
    <a href="https://x.com/{socialLinks.x}" target="_blank" rel="noopener" aria-label="X" class="size-7.5 inline-flex justify-center items-center opacity-50 hover:opacity-100 transition-opacity">
       <svg width="17" height="17" viewBox="0 0 17 17" fill="none"><path d="M17 17L10.352 7.092l.011.01L16.357 0h-2.003l-4.883 5.78L5.594 0H.34l6.207 9.25L0 17h2.003l5.429-6.43L11.747 17H17zM4.8 1.546l9.328 13.908H12.54L3.205 1.546H4.8z" fill="{iconColor}" fill-opacity=".36"/></svg>
    </a>
    {/if}
    ...
    <a href={socialLinks.website.startsWith('http') ? socialLinks.website : `https://${socialLinks.website}`} target="_blank" rel="noopener" aria-label="Website" class="size-7.5 inline-flex justify-center items-center opacity-50 hover:opacity-100 transition-opacity">
       <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><circle cx="8" cy="8" r="7" stroke="{iconColor}" stroke-opacity=".36" stroke-width="1.2"/><path d="M1 8h14M8 1a11 11 0 013 7 11 11 0 01-3 7 11 11 0 01-3-7 11 11 0 013-7z" stroke="{iconColor}" stroke-opacity=".36" stroke-width="1.2"/></svg>
    </a>
    {/if}
   </div>
   {/if}
