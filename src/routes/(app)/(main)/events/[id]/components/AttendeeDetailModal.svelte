<script lang="ts">
	...
	async function loadAttendeeData() {
		...
		try {
			...
			console.error('Failed to load attendee data:', e);
		} finally {
			loading = false;
		}
	}
	...
	async function handleTagAssigned(e: CustomEvent<{ tagName: string }>) {
		...
		} catch (err) { console.error('Failed to assign tag:', err); }
	}
	...
	function getStatusColor(status: string) {
		switch (status) {
			...
			case 'WAITLISTED': return { bg: '#F4ECDA', text: '#D79B1B' };
			case 'DECLINED': return { bg: '#FEF3F2', text: '#D92D20' };
			case 'UNREGISTERED': return { bg: '#DCDCDC', text: '#A9AAAA' };
			case 'UNAPPROVED': return { bg: '#DCDCDC', text: '#A9AAAA' };
	...
	function getTimelineIcon(type: string) {
		...
		if (type === 'status') return { bg: '#ECECEC', color: '#A9AAAA' };
	...
	>
		...
		>
				...
				{:else}
					...
					<div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
						<div class="flex items-center gap-3">
							<div class="flex h-11 w-11 items-center justify-center rounded-full bg-purple-100 text-sm font-medium text-purple-600">
					...
					<!-- Registration Time / Amount / Ticket Type -->
					<div class="mt-5 flex flex-wrap items-center gap-6">
						...
						<div class="border-r border-gray-200 pr-6">
							...
							<p class="text-sm font-semibold text-black">{registration?.payment_status === 'COMPLETED' ? '$0.00' : '$0.00'}</p>
						</div>
						{#if ticketTypeName}
							<div>
								...
								<div class="relative mt-4 flex items-start gap-3">
									...
									<div class="flex-1">
										...
										<p class="mt-1 text-xs text-gray-300 uppercase">
											{formatTime(item.time)}{item.actor ? ` – ${item.actor}` : ''}
										</p>
									</div>
			...
			<div class="flex items-center justify-between border-t border-gray-200 px-6 py-4">
				<button on:click={() => (showReportModal = true)} class="text-xs font-semibold text-[#A6A7A7] transition hover:text-gray-600">
					Report Attendee
				</button>
