<script lang="ts">
	import { createCollectionSubscriberTag, getCollectionSubscriberTags } from '$lib/services/collection.services';
	...
	$: if (open && collectionId) { loadTags(); }

	async function loadTags() {
		loadingTags = true;
		try { existingTags = await getCollectionSubscriberTags(collectionId); } catch { existingTags = []; }
	...
	async function createTag() {
		...
		try {
			const tag = await createCollectionSubscriberTag(collectionId, name.trim(), selectedColor);
	...
	>
		...
		>
			<div class="relative space-y-4 px-6 py-6">
				...
				{#if mode === 'select'}
					<h2 class="text-xl font-semibold text-gray-900">Add Subscriber Tag</h2>
					<p class="text-sm leading-snug text-[#8E8E90]">
						Select an existing tag or create a new one to assign to this subscriber.
					</p>
				...
				{:else}
					<h2 class="text-xl font-semibold text-gray-900">Create Subscriber Tag</h2>
					<p class="text-sm leading-snug text-[#8E8E90]">
						Subscriber tags help you organize and categorize your subscribers.
					</p>
					...
					<div>
						...
						<input
							...
							type="text"
							bind:value={name}
							placeholder="e.g., VIP, Newsletter, Premium"
							class="w-full rounded-lg border border-gray-200 bg-white px-4 py-3 text-sm text-gray-800 placeholder-gray-400 focus:border-gray-400 focus:outline-none"
						/>
					</div>
