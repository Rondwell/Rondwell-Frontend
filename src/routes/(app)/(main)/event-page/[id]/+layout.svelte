<script lang="ts">
	...
	const tabIcons = {
		...
		gifts: `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M20 12v9H4v-9M12 21V8M22 8H2v4h20V8zM12 8H7.5a2.5 2.5 0 010-5C11 3 12 8 12 8zM12 8h4.5a2.5 2.5 0 000-5C13 3 12 8 12 8z" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"/></svg>`,
		memories: `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M23 19a2 2 0 01-2 2H3a2 2 0 01-2-2V8a2 2 0 012-2h4l2-3h6l2 3h4a2 2 0 012 2v11z" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"/><circle cx="12" cy="13" r="4" stroke="currentColor" stroke-width="1.6"/></svg>`
	};
	...
	// Celebration-layer flags, resolved server-side in +layout.server.ts.
	$: features = data?.features ?? null;
	$: giftsEnabled = features?.giftRegistry?.enabled === true;
	$: memoriesEnabled = features?.memories?.enabled === true;
	...
	$: tabs = [
		...
		// GAP 3 — sits right after Participants: once you know you're going,
		// the next question is what to bring.
		...(giftsEnabled
			? [{ label: 'Gifts', link: `/event-page/${eventId}/gifts`, icon: tabIcons.gifts }]
			: []),
		// GAP 10 — deliberately SEPARATE from Media. Media is the organizer's
		// curated gallery; Memories is what the guests shot. Merging them
		// would put unmoderated phone photos next to the sponsor assets.
		{ label: 'Media', link: `/event-page/${eventId}/media`, icon: tabIcons.media },
		...(memoriesEnabled
			? [{ label: 'Memories', link: `/event-page/${eventId}/memories`, icon: tabIcons.memories }]
			: []),
	...
	/**
	 * Sub-pages require sign-in — EXCEPT the ones whose entire purpose is to
	 * serve someone who has no account.
	 *
	 * The Gifts tab is the whole point of a registry: a guest opens the link
	 * their friend sent on WhatsApp and buys something. Putting a sign-in wall
	 * in front of that would kill the feature outright.
	 *
	 * Memories is public-facing too, but gates CONTRIBUTION rather than
	 * VIEWING — the page itself renders a "register to contribute" state, and
	 * the server separately refuses uploads from non-attendees.
	 */
	const PUBLIC_SUB_PAGES = ['/gifts', '/memories'];
	$: isPublicSubPage = PUBLIC_SUB_PAGES.some((suffix) =>
		currentPath.startsWith(`/event-page/${eventId}${suffix}`)
	);
