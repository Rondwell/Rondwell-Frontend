<script lang="ts">
	import { getCollaborations, updateCollaborationStatus, type CollaborationFilters } from '$lib/services/vendor.services';
	import Icon from '@iconify/svelte';
	import { onMount } from 'svelte';
	import InboundOrderDetailModal from './InboundOrderDetailModal.svelte';
	...
	async function fetchOrders() {
		...
		try {
			...
			// Only show INVITATION type (organizer-initiated) — exclude vendor's own SENT_REQUEST
			filters.type = 'INVITATION';
			if (statusFilter) filters.status = statusFilter;
			if (searchQuery) filters.search = searchQuery;
			const result = await getCollaborations(filters);
			orders = result?.collaborations || result?.data || [];
	...
	onMount(() => { fetchOrders(); });
	function handleSearch() { clearTimeout(searchTimeout); searchTimeout = setTimeout(() => { currentPage = 1; fetchOrders(); }, 300); }
	function selectStatus(v: string) { statusFilter = v; showStatusDropdown = false; currentPage = 1; fetchOrders(); }
	function goToPage(p: number) { if (p >= 1 && p <= totalPages) { currentPage = p; fetchOrders(); } }

	async function handleQuickAction(id: string, status: string) {
		actionLoading = id;
		try { await updateCollaborationStatus(id, status); await fetchOrders(); } catch (e) { console.error(e); }
		finally { actionLoading = null; }
	}

	function openDetail(order: any) { selectedOrderId = order._id; selectedOrder = order; showDetailModal = true; }
...
<div>
	<div class="mb-5 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
		<div>
			<h2 class="flex items-center gap-2 text-base font-bold text-gray-900 sm:text-lg">Inbound Order Requests <Icon icon="mdi:information-outline" class="h-4 w-4 text-gray-400" /></h2>
			<p class="mt-0.5 text-xs text-gray-400 sm:text-sm">Review and respond to new requests from Organizers.</p>
		</div>
	</div>
	...
	{:else if orders.length > 0}
			...
			{#each orders as order (order._id)}
				<div class="grid grid-cols-12 items-center gap-2 border-b px-4 py-3 text-sm last:border-b-0 hover:bg-gray-50">
					<div class="col-span-3 flex items-center gap-2 min-w-0">
						<img src={order.creatorId?.avatar || '/rondwell-attendee.png'} alt="" class="h-7 w-7 shrink-0 rounded-full object-cover" />
						<span class="truncate font-medium text-gray-900">{order.organizerName || order.creatorId?.username || 'Organizer'}</span>
					</div>
					<div class="col-span-3 truncate text-gray-600">{order.eventName || order.title || order.eventId || '—'}</div>
					<div class="col-span-2 truncate text-gray-400">{order.category || order.description?.slice(0, 20) || '—'}</div>
					<div class="col-span-1 text-xs text-gray-400">{formatDate(order.createdAt)}</div>
					<div class="col-span-1 text-xs font-medium {getStatusClass(order.status)}">{order.status}</div>
					<div class="col-span-2 flex items-center justify-end gap-1.5">
						<button on:click={() => openDetail(order)} class="rounded-md bg-[#EBECED] px-2.5 py-1 text-[10px] font-medium text-[#616265] hover:bg-gray-300">View Details</button>
		...
		<div class="space-y-2 sm:hidden">
			{#each orders as order (order._id)}
				<button on:click={() => openDetail(order)} class="flex w-full items-center justify-between rounded-lg bg-white p-3 text-left">
					<div class="flex items-center gap-2.5 min-w-0">
						<img src={order.creatorId?.avatar || '/rondwell-attendee.png'} alt="" class="h-8 w-8 shrink-0 rounded-full object-cover" />
						<div class="min-w-0">
							<p class="truncate text-sm font-medium text-gray-900">{order.organizerName || order.creatorId?.username || 'Organizer'}</p>
							<p class="truncate text-xs text-gray-400">{order.eventName || order.title || '—'}</p>
						</div>
					</div>
					<div class="flex flex-col items-end gap-0.5 shrink-0">
						<span class="text-[10px] font-medium {getStatusClass(order.status)}">{order.status}</span>
						<span class="text-[10px] text-gray-400">{formatDate(order.createdAt)}</span>
					</div>
				</button>
			{/each}
		</div>
	...
	{:else}
		<div class="flex flex-col items-center justify-center rounded-2xl border border-gray-100 bg-white py-16 text-center">
			...
			<p class="text-sm font-medium text-gray-500">No inbound requests yet</p>
			<p class="mt-1 text-xs text-gray-400">New requests from organizers will appear here.</p>
		</div>
	{/if}
</div>

<InboundOrderDetailModal bind:open={showDetailModal} collaborationId={selectedOrderId} initialData={selectedOrder} onStatusChanged={() => fetchOrders()} />
