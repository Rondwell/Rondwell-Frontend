<script lang="ts">
	...
	async function fetchRequests() {
		loading = true;
		try {
			const filters: CollaborationFilters = { page: currentPage, limit, sortBy: 'createdAt', sortOrder: 'desc', type: 'SENT_REQUEST' };
...
<div>
	<div class="mb-5 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
		<div>
			<h2 class="flex items-center gap-2 text-base font-bold text-gray-900 sm:text-lg">Sent Collaboration Requests <Icon icon="mdi:information-outline" class="h-4 w-4 text-gray-400" /></h2>
			<p class="mt-0.5 text-xs text-gray-400 sm:text-sm">Track your proactive proposals to Organizers.</p>
		</div>
		<button on:click={openCreateForm} class="flex items-center gap-1.5 rounded-lg bg-black px-4 py-2.5 text-xs font-bold text-white hover:bg-gray-800">
			<Icon icon="mdi:plus" class="h-4 w-4" /> Send New Collaboration Request
		</button>
	</div>
			...
			{#each requests as req (req._id)}
				<div class="grid grid-cols-12 items-center gap-2 border-b px-4 py-3 text-sm last:border-b-0 hover:bg-gray-50">
					<div class="col-span-3 flex items-center gap-2 min-w-0">
						<img src={req.creatorId?.avatar || '/rondwell-attendee.png'} alt="" class="h-7 w-7 shrink-0 rounded-full object-cover" />
						<span class="truncate font-medium text-gray-900">{req.organizerName || req.creatorId?.username || 'Organizer'}</span>
					</div>
					<div class="col-span-3 truncate text-gray-600">{req.eventName || req.title || '—'}</div>
					<div class="col-span-2 truncate text-gray-400">{req.title || req.description?.slice(0, 20) || req.category || '—'}</div>
			...
			{#each requests as req (req._id)}
				<button on:click={() => openDetail(req)} class="flex w-full items-center justify-between rounded-lg bg-white p-3 text-left">
					<div class="flex items-center gap-2.5 min-w-0">
						<img src={req.creatorId?.avatar || '/rondwell-attendee.png'} alt="" class="h-8 w-8 shrink-0 rounded-full object-cover" />
						<div class="min-w-0"><p class="truncate text-sm font-medium text-gray-900">{req.organizerName || req.creatorId?.username || 'Organizer'}</p><p class="truncate text-xs text-gray-400">{req.eventName || req.title || '—'}</p></div>
					</div>
	...
	{:else}
		<div class="flex flex-col items-center justify-center rounded-2xl border border-gray-100 bg-white py-16 text-center">
			...
			<p class="mt-1 text-xs text-gray-400">Your sent collaboration requests will appear here.</p>
		</div>
	{/if}
</div>

<SentRequestDetailModal bind:open={showDetailModal} collaborationId={selectedId} initialData={selectedReq} mode={modalMode} {vendorName} {vendorProfilePictureUrl} onStatusChanged={() => fetchRequests()} />
