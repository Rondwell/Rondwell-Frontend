<script lang="ts">
	...
	import {
		acceptCollectionAdminInvite,
		declineCollectionAdminInvite,
		getCollectionAdminInvitation
	} from '$lib/services/collectionAdmin.services';
	...
	onMount(async () => {
		...
		try {
			invitation = await getCollectionAdminInvitation(collectionId, token);
			status = 'loaded';
			...
			if (e.code === 'EXPIRED') {
				status = 'expired';
				message = 'This invitation link has expired. Ask the owner to resend the invite.';
	...
	function redirectTarget(): string {
		return `/collection-admin-invitation/${token}?collectionId=${collectionId}&action=accept`;
	}
	...
	async function handleAccept() {
		...
		if (browser && !$isAuthenticated) {
			...
			goto(`/auth?returnUrl=${encodeURIComponent(redirectTarget())}`);
			return;
		}

		status = 'accepting';
		try {
			await acceptCollectionAdminInvite(collectionId, token);
			status = 'accepted';
			message = 'Welcome aboard! Redirecting you to the collection…';
			setTimeout(() => {
	...
	async function handleDecline() {
		...
		status = 'declining';
		try {
			await declineCollectionAdminInvite(collectionId, token);
			status = 'declined';
			message = 'You have declined this invitation. The owner will be notified.';
		} catch (e: any) {
			status = 'error';
...
<svelte:head>
	<title
		>{invitation?.collection?.name
			? `Collection Admin Invitation — ${invitation.collection.name}`
			: 'Collection Admin Invitation'} — Rondwell</title
	>
</svelte:head>
	...
	<div class="relative z-10 mx-auto flex w-full max-w-lg flex-col items-center text-center">
		{#if status === 'loading'}
			<div class="status-icon-wrapper" class:mounted>
				<div
					class="flex h-24 w-24 items-center justify-center rounded-full bg-white/80 shadow-lg shadow-purple-500/10"
				>
					<div
						class="h-10 w-10 animate-spin rounded-full border-4 border-gray-200 border-t-[#DB3EC6]"
					></div>
				</div>
			</div>
		...
		{:else if (status === 'loaded' || status === 'accepting' || status === 'declining') && invitation}
			<div class="status-icon-wrapper" class:mounted>
				...
				>
					<span class="text-2xl font-semibold text-[#DB3EC6]">
						{getInitials(
							invitation.displayName || invitation.email || invitation.organizerName || ''
						)}
					</span>
				</div>
			</div>
			...
			<div class="message-entrance" class:mounted>
				<p class="mb-1 text-xs font-medium uppercase tracking-wider text-[#DB3EC6]">
					Collection Admin Invitation
				</p>
				<h1 class="mb-2 text-2xl font-semibold text-[#131517]">You're Invited to Co-manage</h1>
			...
			<div class="actions-entrance w-full" class:mounted>
				...
				>
					{#if invitation.collection?.coverImageUrl}
						<div class="h-36 w-full overflow-hidden">
							<img
								src={invitation.collection.coverImageUrl}
								alt={invitation.collection.name}
					...
					{:else}
						...
						>
							<Icon icon="mdi:folder-star-outline" class="h-10 w-10 text-[#DB3EC6]/30" />
						</div>
					{/if}

					<div class="p-5">
						<h2 class="mb-3 text-lg font-bold text-[#131517]">
							{invitation.collection?.name || 'Collection'}
						</h2>
						...
						<div class="space-y-2.5">
							{#if invitation.organizerName}
								<div class="flex items-center gap-2.5">
									<div
										class="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-green-50"
									>
										<Icon icon="mdi:account-outline" class="h-4 w-4 text-green-500" />
									</div>
									<div>
										<p class="text-xs text-[#919091]">Managed by</p>
							...
							{#if invitation.expiresAt}
								<div class="flex items-center gap-2.5">
									<div
										class="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-orange-50"
									>
										<Icon icon="mdi:clock-outline" class="h-4 w-4 text-orange-500" />
									</div>
									<div>
										<p class="text-xs text-[#919091]">Expires</p>
										<p class="text-sm font-medium text-[#131517]">
		...
		{:else if status === 'accepted'}
			<div class="status-icon-wrapper" class:mounted>
				<div
					class="flex h-24 w-24 items-center justify-center rounded-full bg-white/80 shadow-lg shadow-green-500/10"
				>
		...
		{:else if status === 'declined'}
			<div class="status-icon-wrapper" class:mounted>
				<div
					class="flex h-24 w-24 items-center justify-center rounded-full bg-white/80 shadow-lg shadow-gray-500/10"
				>
		...
		{:else if status === 'already_processed'}
			<div class="status-icon-wrapper" class:mounted>
				<div
					class="flex h-24 w-24 items-center justify-center rounded-full bg-white/80 shadow-lg shadow-blue-500/10"
				>
		...
		{:else if status === 'expired'}
			<div class="status-icon-wrapper" class:mounted>
				<div
					class="flex h-24 w-24 items-center justify-center rounded-full bg-white/80 shadow-lg shadow-yellow-500/10"
				>
		...
		{:else if status === 'wrong_account'}
			<div class="status-icon-wrapper" class:mounted>
				<div
					class="flex h-24 w-24 items-center justify-center rounded-full bg-white/80 shadow-lg shadow-orange-500/10"
				>
			...
			<div class="actions-entrance flex gap-3" class:mounted>
				<a
					href={`/auth?returnUrl=${encodeURIComponent(redirectTarget())}`}
					class="inline-flex h-12 items-center gap-2 rounded-xl bg-[#333537] px-7 text-sm text-white transition-all duration-300 hover:bg-gray-900"
				>
					Switch account
				</a>
		...
		{:else}
			<div class="status-icon-wrapper" class:mounted>
				<div
					class="flex h-24 w-24 items-center justify-center rounded-full bg-white/80 shadow-lg shadow-red-500/10"
				>
...
<style>
	...
	.grid-overlay {
		position: absolute;
		inset: 0;
		background-image: linear-gradient(rgba(219, 62, 198, 0.03) 1px, transparent 1px),
