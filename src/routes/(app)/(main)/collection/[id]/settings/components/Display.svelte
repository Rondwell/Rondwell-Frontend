					...
					>
						...
						<div class="relative z-10">
							...
							{#if search}
								<button
									aria-label="button"
									class="absolute right-3 top-1.5 text-gray-400"
									on:click={() => (search = '')}
								>
									<svg
										width="26"
				...
				{#each socialLinksMeta as { key, icon, label, prefix }}
					<div class="flex items-center gap-2">
						...
						<div class="flex h-[42px] w-full items-center shadow-sm">
							...
							{#if label}
								<div
									class="pointer-events-none flex h-full items-center rounded-bl-[7.5px] rounded-tl-[7.5px] bg-[#EBECED] p-3 text-xs text-[#84857A]"
								>
									{label}
								</div>
							{/if}
							...
							<input
								...
								class="h-full w-full {key === 'website'
									? 'rounded-[7.5px]'
									: 'rounded-br-[7.5px] rounded-tr-[7.5px]'} bg-[#FFFFFF] px-3 py-2 focus:outline-none focus:ring-0"
							/>
						</div>
					</div>
				{/each}
