	...
	onMount(() => {
		const unsub = isAuthenticated.subscribe(async (authed) => {
			...
			if (browser && authed) {
				...
				if (currentPath.startsWith('/vendor/onboarding')) {
					checked = true;
					return;
				}
				...
				try {
					...
					const vendorProfile = profiles.find((p) => p.role === 'VENDOR');
					if (!vendorProfile || vendorProfile.onboardingStatus !== 'COMPLETED') {
						goto('/vendor/onboarding');
						return;
					}
				} catch {
