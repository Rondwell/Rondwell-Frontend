<script lang="ts">
	...
	onMount(async () => {
		try {
			...
			exhibitorProfileSlug = profile?.publicProfileSlug || '';
		} catch {
			// silent
		}
		await loadBooths();
				...
				{#each booths as booth (booth._id)}
					<div class="group relative rounded-xl border border-gray-100 bg-white p-3 shadow-sm transition hover:shadow-md">
						...
						<div class="mt-3 flex items-center gap-2">
							...
							{#if exhibitorProfileSlug && booth.slug && booth.status === 'PUBLISHED'}
								<a href="/x/{exhibitorProfileSlug}/{booth.slug}" target="_blank" rel="noopener noreferrer"
