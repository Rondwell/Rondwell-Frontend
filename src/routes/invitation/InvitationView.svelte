<!--
  Shared invitation view. Rendered by BOTH `/invitation/[token]` (H-05, the
  path-segment form the emails now use) and `/invitation?token=…` (the legacy
  query form, kept alive for links already sitting in inboxes).

  The token is a PROP rather than something this component reads from the URL,
  so neither route can drift from the other.
-->
<script lang="ts">
	...
	/** H-05 — the raw invite token, supplied by whichever route rendered us. */
