<script lang="ts">
	...
	import {
		...
		getCollectionRoleColor,
		getCollectionRoleLabel
	} from '$lib/constants/collection-admin-roles';
	import { getCollectionAdmins } from '$lib/services/collectionAdmin.services';
	...
	import AddCollectionAdmin from './modal/AddCollectionAdmin.svelte';
	import UpdateCollectionAdmin from './modal/UpdateCollectionAdmin.svelte';
	...
	async function loadAdmins() {
		...
		try {
			const [adminList, profile, me] = await Promise.all([
				getCollectionAdmins(collectionId),
			...
			admins = adminList.map((a: any) => {
				...
				const needsName =
					!a.displayName || a.displayName === 'User' || /^[a-f\d]{24}$/i.test(a.displayName);
			...
			console.error('Failed to load collection admins:', e);
			admins = [];
		} finally {
			loading = false;
		}
	...
	function getRoleInfo(admin: any): { label: string; color: string } {
		const c = getCollectionRoleColor(admin.role);
		return { label: getCollectionRoleLabel(admin.role), color: `${c.bg} ${c.text}` };
	}
...
<section class="px-1 py-4 sm:p-4">
	<div class="mb-3 flex flex-col justify-between gap-2 md:flex-row md:items-center">
		<span>
			...
			<p class="text-sm text-[#737577]">
				Admins help manage this collection and the events under it.
			</p>
		</span>
		<button
			on:click={() => (showAddModal = true)}
			class="flex w-full items-center justify-center gap-1 rounded-md bg-[#EBECED] px-3 py-2 text-xs font-medium text-[#616265] transition-colors hover:bg-gray-200 sm:w-fit md:text-sm"
		>
	...
	<div class="mb-4 flex items-center gap-3">
		<div class="relative w-full">
			<input
				...
				placeholder="Search"
				class="h-[43px] w-full rounded-lg bg-white py-2 pl-10 pr-4 text-[#C5C6C6] focus:outline-none focus:ring-0"
			/>
			...
			<span class="absolute right-0 top-2.5 border-l px-4 py-1.5 text-gray-400"
				><img src="/search-download.svg" alt="" class="h-4 w-4" /></span
			>
		</div>
	</div>
	...
	<div class="mb-4 flex items-center justify-between">
		<button
			class="hidden flex-shrink-0 items-center gap-2 rounded-md bg-[#EBECED] px-3 py-2 text-xs text-[#616265] md:flex md:text-sm"
		>
			<img src="/filter-edit.svg" alt="" class="h-5 w-5" /> Filter
			<img src="/arrow-down.svg" alt="" class="h-2 w-3" />
		</button>
		<div class="flex items-center gap-3">
			<div use:clickOutside={() => (showRoleFilter = false)} class="relative">
				<button
					on:click={() => (showRoleFilter = !showRoleFilter)}
					class="flex flex-shrink-0 items-center gap-2 rounded-md bg-[#EBECED] px-3 py-2 text-xs text-[#616265] md:text-sm"
				>
					<img src="/filter-edit.svg" alt="" class="h-5 w-5" />
					{roleFilter === 'All' ? 'All Admin' : roleLabels[roleFilter]?.label || roleFilter}
					<img src="/arrow-down.svg" alt="" class="h-2 w-3" />
				</button>
				{#if showRoleFilter}
					<div
						class="absolute right-0 z-50 mt-2 w-52 rounded-xl bg-white p-2 text-[15px] text-[#768387] shadow-lg"
					>
						<button
							on:click={() => {
								roleFilter = 'All';
							...
							class="flex w-full items-center gap-2 rounded-md p-2 hover:bg-[#EBECED] {roleFilter ===
							'All'
								? 'bg-[#EBECED]'
								: ''}"><Icon icon="mdi:view-grid-outline" class="text-lg" /> All Admin</button
						>
						{#each Object.entries(roleLabels) as [key, val]}
							<button
								...
								key
									? 'bg-[#EBECED]'
									: ''}"><Icon icon="mdi:account-outline" class="text-lg" /> {val.label}</button
							>
						{/each}
					</div>
				{/if}
			...
			<div use:clickOutside={() => (showSortFilter = false)} class="relative">
				<button
					on:click={() => (showSortFilter = !showSortFilter)}
					class="flex flex-shrink-0 items-center gap-2 rounded-md bg-[#EBECED] px-3 py-2 text-xs text-[#616265] md:text-sm"
				>
					<img src="/filter-edit.svg" alt="" class="h-5 w-5" />
					{sortOrder === 'recent'
				...
				{#if showSortFilter}
					...
					>
						<button
							...
							class="flex w-full items-center gap-2 rounded-md p-2 hover:bg-[#EBECED] {sortOrder ===
							'recent'
								? 'bg-[#EBECED]'
								: ''}"
							><Icon icon="mdi:sort-clock-descending" class="text-lg" /> Recently Joined</button
						>
						<button
							on:click={() => {
								sortOrder = 'oldest';
							...
							'oldest'
								? 'bg-[#EBECED]'
								: ''}"
							><Icon icon="mdi:sort-clock-ascending" class="text-lg" /> Oldest First</button
						>
						<button
							on:click={() => {
								sortOrder = 'name';
							...
							'name'
								? 'bg-[#EBECED]'
								: ''}"
							><Icon icon="mdi:sort-alphabetical-ascending" class="text-lg" /> By Name</button
						>
					</div>
				{/if}
			</div>
...
<AddCollectionAdmin bind:open={showAddModal} {collectionId} on:saved={handleSaved} />
