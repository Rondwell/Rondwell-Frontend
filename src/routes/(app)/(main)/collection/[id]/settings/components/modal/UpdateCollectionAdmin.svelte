<script lang="ts">
	...
	import {
		...
		resendCollectionAdminInvite,
		updateCollectionAdminPermissions
	...
	$: if (open && admin) {
		selectedRole = admin.role || 'COLLECTION_MANAGER';
		showOnCollectionPage = admin.showOnCollectionPage ?? true;
		error = '';
		resendSuccess = '';
	}
	...
	async function handleUpdate() {
		...
		error = '';
		try {
			await updateCollectionAdminPermissions(collectionId, admin.id, {
				role: selectedRole,
	...
	async function handleRemove() {
		if (!admin?.id) return;
		if (!confirm('Remove this admin? They will lose access to manage this collection.')) return;
		try {
			await removeCollectionAdmin(collectionId, admin.id);
			dispatch('saved');
			open = false;
		} catch (e: any) {
	...
	async function handleResendInvite() {
		...
		error = '';
		try {
			await resendCollectionAdminInvite(collectionId, admin.id);
			...
			<div class="custom-scrollbar max-h-[80vh] overflow-y-auto px-6 py-6">
				<div class="mb-1 flex items-center justify-between">
					<div class="flex h-12 w-12 items-center justify-center rounded-lg bg-gray-100">
						<Icon icon="mdi:account-cog-outline" class="text-2xl text-gray-500" />
					</div>
				...
				{/if}
				{#if resendSuccess}
					<p class="mt-3 rounded-md bg-green-50 p-3 text-sm text-green-600">{resendSuccess}</p>
				{/if}

				<!-- Resend Invitation (only for pending admins) -->
				{#if admin.status === 'PENDING'}
					<div
						class="mt-4 flex items-center justify-between rounded-lg border border-orange-200 bg-orange-50 p-4"
					>
						<div>
						...
						<button
							...
							disabled={resending}
							class="flex-shrink-0 rounded-lg bg-orange-600 px-3 py-2 text-xs font-medium text-white disabled:opacity-50"
						>
				...
				<div class="mt-5 flex items-center gap-3">
					...
					<button
						...
						class="rounded-lg border border-red-300 px-4 py-3 text-sm font-medium text-red-500 hover:bg-red-50"
					>
						Remove Admin
					</button>
				</div>
