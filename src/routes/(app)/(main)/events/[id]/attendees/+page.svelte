<script lang="ts">
	...
	function getStatusClass(status: string): string {
		switch (status) {
			case 'ATTENDING':
			case 'CHECKED_IN':
				return 'bg-[#E3F4E1] text-[#3CBD2C]';
			case 'PENDING':
			case 'WAITLISTED':
				return 'bg-[#FFF8E1] text-[#EAAB26]';
			case 'DECLINED':
				return 'bg-[#FDEAEA] text-[#E53935]';
			default:
				return 'bg-[#EBECED] text-[#616265]';
		}
	}
	...
	async function handleToggleGuestList(show: boolean) {
		if (!eventData) return;
		try {
			await updateEvent(eventId, { publicGuestListEnabled: show });
			eventData.publicGuestListEnabled = show;
		} catch {
			// silently fail
		}
	}
