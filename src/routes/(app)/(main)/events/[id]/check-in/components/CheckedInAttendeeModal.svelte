<script lang="ts">
	...
	$: if (open && attendeeId && eventId) { loadData(); }

	async function loadData() {
		loading = true;
		try {
			const [detail, tl] = await Promise.all([
				getAttendeeDetail(eventId, attendeeId),
				getAttendeeTimeline(eventId, attendeeId)
			]);
	...
	async function handleUndoCheckIn() {
		...
		try {
			...
			await loadData();
		} catch (e) { console.error('Failed to undo check-in:', e); }
	...
	async function handleTagAssigned(e: CustomEvent<{ tagName: string }>) {
		try {
			...
			if (!attendee.tags.includes(e.detail.tagName)) attendee.tags = [...attendee.tags, e.detail.tagName];
		} catch (err) { console.error(err); }
	}

	function handleStatusUpdated() { loadData(); }

	function getStatusColor(s: string) {
		switch (s) {
			case 'CHECKED_IN': return { bg: '#E3F4E1', text: '#3CBD2C' };
			case 'ATTENDING': return { bg: '#E3F4E1', text: '#3CBD2C' };
			case 'PENDING': return { bg: '#F4ECDA', text: '#D79B1B' };
			case 'DECLINED': return { bg: '#FEF3F2', text: '#D92D20' };
			default: return { bg: '#DCDCDC', text: '#A9AAAA' };
		}
	}
	function getStatusLabel(s: string) {
		const map: Record<string,string> = { CHECKED_IN:'Checked-In', ATTENDING:'Attending', PENDING:'Pending', WAITLISTED:'Waitlisted', DECLINED:'Declined', UNREGISTERED:'Unregistered', UNAPPROVED:'Unapproved' };
		return map[s] || s;
	}
...
{#if open}
	...
	>
		<div class="flex h-full w-full max-w-xl flex-col rounded-xl bg-[#F4F5F6] shadow-xl lg:max-w-lg lg:rounded-none" role="document" on:click|stopPropagation on:keydown|stopPropagation>
			<!-- Header -->
			...
			<div class="custom-scrollbar flex-1 overflow-y-auto px-6 pt-5 pb-6">
				{#if loading}
					<div class="flex items-center justify-center py-20"><Icon icon="mdi:loading" class="animate-spin text-3xl text-gray-400" /></div>
				{:else}
					<!-- Attendee Info -->
					...
					<div class="mt-5 flex flex-wrap items-center gap-2">
						<button class="flex items-center rounded-2xl bg-[#EFEFEF] px-2 py-0.5 text-xs text-[#A0A1A3] transition hover:bg-gray-200" on:click={() => (showCreateTagModal = true)}>
					...
					{#if formAnswers.length > 0}
						<div class="mt-6 border-t border-gray-200 pt-6">
							<div class="flex items-center justify-between">
								...
								<button class="flex items-center gap-1 rounded-md bg-[#EFEFEF] px-3 py-1 text-xs text-[#A9AAAA] transition hover:bg-gray-200"><img src="/edit.svg" alt="" class="h-3 w-3" /> Edit</button>
							</div>
