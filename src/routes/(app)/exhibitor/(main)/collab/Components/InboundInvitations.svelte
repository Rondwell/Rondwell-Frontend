<script lang="ts">
	...
	async function fetchInvitations() {
		loading = true;
		try {
			const filters: ExhibitorCollaborationFilters = { page: currentPage, limit, sortBy: 'createdAt', sortOrder: 'desc', type: 'INVITATION' };
...
<div>
	<div class="mb-5 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
		<div>
			...
			<p class="mt-0.5 text-xs text-gray-400 sm:text-sm">Review and respond to exhibition invitations from Organizers.</p>
		</div>
	</div>
		...
		<div class="hidden rounded-lg bg-white sm:block">
			{#each invitations as inv (inv._id)}
				<div class="grid grid-cols-12 items-center gap-2 border-b px-4 py-3 text-sm last:border-b-0 hover:bg-gray-50">
					...
					<div class="col-span-2 truncate text-gray-400">{inv.packageName || inv.category || '—'}</div>
	...
	{:else}
		<div class="flex flex-col items-center justify-center rounded-2xl border border-gray-100 bg-white py-16 text-center">
			...
			<p class="mt-1 text-xs text-gray-400">Invitations from organizers will appear here.</p>
		</div>
	{/if}
</div>
