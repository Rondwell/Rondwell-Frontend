<!--
	FE-P4-02 — Top organizers by commission generated.
-->
<script lang="ts">
	import {
		downloadBlob,
	...
	async function load() {
		...
		error = '';
		try {
			rows = await getTopOrganizers({ from, to, limit: 50 });
		} catch (e) {
	...
	async function handleExport() {
		exporting = true;
		try {
			const blob = await exportFinanceCsv('organizers', { from, to });
			downloadBlob(blob, `top-organizers-${from}_to_${to}.csv`);
		} catch (e) {
			error = financialErrorMessage(e);
		} finally {
			exporting = false;
...
<svelte:head><title>Top organizers — Rondwell HQ</title></svelte:head>
...
<div>
	...
	<div class="mt-4 flex flex-wrap items-center justify-between gap-3">
		<div>
			<h1 class="text-2xl font-semibold text-gray-900">Top organizers</h1>
			...
			<table class="min-w-full divide-y divide-gray-100 text-sm">
				<thead class="bg-gray-50 text-xs uppercase text-gray-500">
					<tr>
						...
						<th class="px-4 py-2 text-left font-medium">Organizer</th>
