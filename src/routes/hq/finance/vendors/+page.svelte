<!--
	FE-P4-02 — Top vendors by commission generated.
-->
<script lang="ts">
	import {
		downloadBlob,
	...
	async function load() {
		...
		error = '';
		try {
			rows = await getTopVendors({ from, to, limit: 50 });
		} catch (e) {
	...
	async function handleExport() {
		exporting = true;
		try {
			const blob = await exportFinanceCsv('vendors', { from, to });
			downloadBlob(blob, `top-vendors-${from}_to_${to}.csv`);
		} catch (e) {
			error = financialErrorMessage(e);
		} finally {
			exporting = false;
...
<svelte:head><title>Top vendors — Rondwell HQ</title></svelte:head>
...
<div>
	...
	<div class="mt-4 flex flex-wrap items-center justify-between gap-3">
		<div>
			<h1 class="text-2xl font-semibold text-gray-900">Top vendors</h1>
