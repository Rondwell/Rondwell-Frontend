<script lang="ts">
	...
	$: if (open && admin) {
		selectedRole = admin.role || 'EVENT_MANAGER';
		showOnEventPage = admin.showOnEventPage ?? true;
		error = '';
		resendSuccess = '';
	}
	...
	async function handleUpdate() {
		...
		error = '';
		try {
			await updateEventAdminPermissions(eventId, admin.id, {
				role: selectedRole,
				showOnEventPage
			});
			dispatch('saved');
			...
			<div class="custom-scrollbar max-h-[80vh] overflow-y-auto px-6 py-6">
				<div class="mb-1 flex items-center justify-between">
					<div class="flex h-12 w-12 items-center justify-center rounded-lg bg-gray-100">
						<img src="/add admin.svg" alt="admin" class="h-10 w-10" />
					</div>
