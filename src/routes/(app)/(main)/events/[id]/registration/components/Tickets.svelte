<script lang="ts">
	...
	import CreateTicketModal from './modal/CreateTicketModal.svelte';
	...
	import RegistrationStatusModal from './modal/RegistrationStatusModal.svelte';
	...
	let showRegistrationStatusModal = false;
	...
	async function fetchTickets() {
		ticketsLoading = true;
		try {
			tickets = await getTicketTypes(eventId);
		} catch (e: any) {
	...
	async function handleToggleRequireApproval(ticket: any) {
		const newVal = !ticket.requiresApproval;
		try {
			await updateTicketType(eventId, ticket.id, { requiresApproval: newVal });
			ticket.requiresApproval = newVal;
			tickets = [...tickets];
		} catch (e: any) {
			console.error('Failed to toggle approval:', e.message);
		}
	}
	...
	async function handleRegistrationStatusChange(isOpen: boolean) {
		try {
			const updated = await updateRegistrationOpen(eventId, isOpen);
			registrationOpen = isOpen;
			eventData.registrationOpen = isOpen;
			showRegistrationStatusModal = false;
		} catch (e: any) {
			console.error('Failed to update registration status:', e.message);
		}
	}

	async function handleCapacityUpdate(capacity: number | null, waitlist: boolean) {
		try {
			await updateEventCapacity(eventId, capacity, waitlist);
			maxAttendees = capacity ?? 0;
			waitlistEnabled = waitlist;
			eventData.maxAttendees = capacity;
			eventData.waitlistEnabled = waitlist;
			showCapacityModal = false;
		} catch (e: any) {
			console.error('Failed to update capacity:', e.message);
		}
	}
	...
	function getTicketPriceLabel(ticket: any): string {
		if (ticket.isFree || !ticket.price || ticket.price === 0) return 'Free';
		return `${ticket.currency ?? 'USD'} ${ticket.price.toLocaleString()}`;
	}

	function getTicketStatusLabel(ticket: any): string {
		const now = new Date();
		if (ticket.salesEndDate && new Date(ticket.salesEndDate) < now) return 'Ended';
		if (ticket.salesStartDate && new Date(ticket.salesStartDate) > now) return 'Upcoming';
		return 'Available';
	}

	function getTicketStatusColor(ticket: any): string {
		const status = getTicketStatusLabel(ticket);
...
<div class="">
	...
	<div class="mb-6 flex w-full flex-wrap gap-3">
		...
		<div class="relative w-full md:w-fit" use:clickOutside={() => { showRegistrationStatusModal = false; }}>
			<button
				on:click={() => (showRegistrationStatusModal = !showRegistrationStatusModal)}
			...
			>
				<div class="flex h-[44px] w-[44px] items-center justify-center rounded-sm bg-[#DBF4DA]">
					<svg width="29" height="27" viewBox="0 0 29 27" fill="none" xmlns="http://www.w3.org/2000/svg">
						...
						<path d="M15.4766 8.71819L17.8572 10.9038L22.9791 6.55469" stroke="white" stroke-width="1.49844" stroke-linecap="round" stroke-linejoin="round"/>
					</svg>
				</div>
				<div>
					<p>Registration</p>
					<p class="text-left text-xs text-[#B8BABA]">{registrationOpen ? 'Open' : 'Closed'}</p>
				</div>
			</button>
			<RegistrationStatusModal
				bind:open={showRegistrationStatusModal}
				{registrationOpen}
				onConfirm={handleRegistrationStatusChange}
			/>
		</div>

		<!-- Event Capacity Button -->
		<div class="relative w-full md:w-fit" use:clickOutside={() => { showCapacityModal = false; }}>
			<button
			...
			>
				<div class="flex h-[44px] w-[44px] items-center justify-center rounded-sm bg-[#F8E8E0]">
					<svg width="28" height="27" viewBox="0 0 28 27" fill="none" xmlns="http://www.w3.org/2000/svg">
						<rect y="4.49219" width="4.49532" height="27.7461" rx="2.24766" transform="rotate(-90 0 4.49219)" fill="#F87937"/>
						<rect x="11.4258" y="14.2344" width="4.89637" height="12.7367" rx="2.44819" fill="#F87937"/>
						<path d="M13.3 5.92689C13.5994 5.56977 14.1488 5.56977 14.4482 5.92689L22.7365 15.8118C23.1453 16.2994 22.7987 17.0424 22.1624 17.0424H5.58581C4.94953 17.0424 4.60288 16.2994 5.01169 15.8118L13.3 5.92689Z" fill="#F87937"/>
					</svg>
				</div>
				<div>
					<p>Event Capacity</p>
					<p class="text-left text-xs text-[#B8BABA]">{maxAttendees > 0 ? maxAttendees : 'Unlimited'}</p>
				</div>
			</button>
			<CapacityModal
				bind:open={showCapacityModal}
				currentCapacity={maxAttendees}
				currentWaitlist={waitlistEnabled}
				onSetLimit={handleCapacityUpdate}
			/>
		</div>

		<!-- Group Registration Button -->
		<div class="relative w-full md:w-fit" use:clickOutside={() => { showGroupRegisterModal = false; }}>
			<button
			...
			>
				<div class="flex h-[44px] w-[44px] items-center justify-center rounded-sm bg-[#F2E4F8]">
					<svg width="29" height="27" viewBox="0 0 29 27" fill="none" xmlns="http://www.w3.org/2000/svg">
						<path opacity="0.4" d="M24.8037 14.3892C25.2678 14.3892 25.6367 14.0396 25.6367 13.6025V12.641C25.6367 8.34708 24.2086 7.04688 19.5435 7.04688H12.0459V9.70192C12.51 9.70192 12.8909 10.0516 12.8909 10.4777V13.4059C12.8909 13.832 12.51 14.1816 12.0459 14.1816V16.9241C12.51 16.9241 12.8909 17.2737 12.8909 17.6998V20.628C12.8909 21.0542 12.51 21.4038 12.0459 21.4038V24.037H19.5435C24.2086 24.037 25.6367 22.7259 25.6367 18.4428C25.6367 18.0167 25.2678 17.6671 24.8037 17.6671C23.804 17.6671 23.0066 16.935 23.0066 16.0281C23.0066 15.1213 23.804 14.3892 24.8037 14.3892Z" fill="#AB46DD"/>
						<path opacity="0.4" d="M18.127 7.28819H10.709L13.1 5.09298C15.0504 3.30234 16.0297 3.30234 17.9801 5.09298L18.4697 5.54251C17.9556 6.01452 17.8332 6.71129 18.127 7.28819Z" stroke="#AB46DD" stroke-width="1.12383" stroke-linecap="round" stroke-linejoin="round"/>
						<path d="M11.1998 10.4746V13.4028C11.1998 13.8289 11.5807 14.1785 12.0448 14.1785V16.921C11.5807 16.921 11.1998 17.2706 11.1998 17.6967V20.6249C11.1998 21.051 11.5807 21.4007 12.0448 21.4007V24.0339H9.02197C4.35682 24.0339 2.92871 22.7227 2.92871 18.4397V17.9699C2.92871 17.5328 3.29764 17.1941 3.76177 17.1941C4.76145 17.1941 5.55881 16.4511 5.55881 15.5443C5.55881 14.6374 4.76145 13.8944 3.76177 13.8944C3.29764 13.8944 2.92871 13.5557 2.92871 13.1187V12.6489C2.92871 8.3549 4.35682 7.05469 9.02197 7.05469H12.0329V9.70973C11.5807 9.70973 11.1998 10.0594 11.1998 10.4746Z" fill="#AB46DD"/>
					</svg>
				</div>
				<div>
	...
	<div class="mb-6">
		<div class="mb-4 flex items-center justify-between">
			...
			<div class="flex items-center gap-2">
				<button
					on:click={() => { editingTicket = null; showCreateTicketModal = true; }}
				...
				<button aria-label="layout" class="rounded bg-[#EBECED] p-2" on:click={toggleLayout}>
					{#if layout !== 'grid'}
						<svg width="17" height="17" viewBox="0 0 17 17" fill="none" xmlns="http://www.w3.org/2000/svg">
							<path d="M3.4334 6.86928H4.80697C6.18054 6.86928 6.86732 6.18249 6.86732 4.80892V3.43535C6.86732 2.06178 6.18054 1.375 4.80697 1.375H3.4334C2.05983 1.375 1.37305 2.06178 1.37305 3.43535V4.80892C1.37305 6.18249 2.05983 6.86928 3.4334 6.86928Z" stroke="#626265" stroke-width="1.12383" stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round"/>
							<path d="M11.6746 6.86928H13.0482C14.4218 6.86928 15.1085 6.18249 15.1085 4.80892V3.43535C15.1085 2.06178 14.4218 1.375 13.0482 1.375H11.6746C10.301 1.375 9.61426 2.06178 9.61426 3.43535V4.80892C9.61426 6.18249 10.301 6.86928 11.6746 6.86928Z" stroke="#626265" stroke-width="1.12383" stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round"/>
							<path d="M11.6746 15.1115H13.0482C14.4218 15.1115 15.1085 14.4247 15.1085 13.0511V11.6775C15.1085 10.304 14.4218 9.61719 13.0482 9.61719H11.6746C10.301 9.61719 9.61426 10.304 9.61426 11.6775V13.0511C9.61426 14.4247 10.301 15.1115 11.6746 15.1115Z" stroke="#626265" stroke-width="1.12383" stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round"/>
							<path d="M3.4334 15.1115H4.80697C6.18054 15.1115 6.86732 14.4247 6.86732 13.0511V11.6775C6.86732 10.304 6.18054 9.61719 4.80697 9.61719H3.4334C2.05983 9.61719 1.37305 10.304 1.37305 11.6775V13.0511C1.37305 14.4247 2.05983 15.1115 3.4334 15.1115Z" stroke="#626265" stroke-width="1.12383" stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round"/>
						</svg>
					{:else}
		...
		<div class="mb-4 rounded-lg border p-4">
			<div class="flex flex-col items-start justify-between gap-3 lg:flex-row lg:items-center">
				...
				<div class="flex w-full items-center gap-2 lg:w-fit">
					<a href="/auth" class="flex w-full items-center justify-center gap-1 rounded-md bg-[#EBECED] px-3 py-2 text-xs font-medium text-[#616265] transition-colors hover:bg-gray-200 md:w-fit md:text-sm">
						Get Started
					</a>
					<button><Icon icon="mdi:close" class="h-4 w-4 text-gray-600" /></button>
				</div>
			</div>
		</div>
		...
		{#if ticketsLoading}
			...
			<div class="flex flex-wrap gap-4 animate-pulse">
				{#each [1, 2] as _}
		...
		{:else if tickets.length > 0}
						...
						<div class="w-full rounded-lg bg-[#FDFDFD] p-4 md:max-w-[400px]">
								...
								<div class="flex flex-wrap items-center gap-2">
									...
									{#if ticket.tags?.length}
										{#each ticket.tags as tag}
											<span class="rounded-full bg-[#F8EFDD] px-2 py-0.5 text-xs font-medium text-[#D69814]">{tag}</span>
										{/each}
									{/if}
								</div>
								<div class="relative" use:clickOutside={() => { if (menuOpen === ticket.id) menuOpen = null; }}>
									<button on:click={() => toggleMenu(ticket.id)} class="rounded p-1 hover:bg-gray-100">
										<Icon icon="mdi:dots-horizontal" class="h-5 w-5 text-gray-500" />
									</button>
									{#if menuOpen === ticket.id}
										<div class="absolute right-0 z-50 mt-2 w-35 rounded-md border border-gray-200 bg-white p-1 shadow-md">
											<button on:click={() => openEditTicket(ticket)} class="flex w-full items-center gap-2 px-3 py-2 text-left text-sm hover:bg-[#F0F0F0]">
												<Icon icon="mdi:pencil-outline" class="h-4 w-4 text-gray-500" /> Edit
											</button>
											<button on:click={() => { showDeleteTicket = ticket.id; menuOpen = null; }} class="flex w-full items-center gap-2 px-3 py-2 text-left text-sm hover:bg-[#F0F0F0]">
												<Icon icon="mdi:trash-can-outline" class="h-4 w-4" /> Delete
											</button>
										</div>
									{/if}
								</div>
							</div>
							<div class="mb-2 text-2xl font-bold">{getTicketPriceLabel(ticket)}</div>
							...
							<div class="mb-3 flex items-center justify-between">
								...
								<button
									...
									class:bg-gray-300={!ticket.requiresApproval}
									class:bg-gray-800={ticket.requiresApproval}
									on:click={() => handleToggleRequireApproval(ticket)}
								>
									<span
										class="absolute top-0.5 left-0.5 h-5 w-5 rounded-full bg-white transition-transform duration-300"
										class:translate-x-4={ticket.requiresApproval}
									></span>
								</button>
							</div>
							<!-- Status -->
							<div class="mt-3 flex items-center justify-between border-t pt-3">
								<div class="flex items-center gap-2">
									<div class="flex items-center gap-1">
										<div class="h-2 w-2 rounded-full {getTicketStatusColor(ticket)}"></div>
										<span class="text-sm text-gray-500">{getTicketStatusLabel(ticket)}</span>
									</div>
								...
								<div class="flex items-center gap-1.5">
									<div class="flex -space-x-1.5">
										<img src="/rondwell-attendee.png" alt="Attendee" class="h-5 w-5 rounded-full border border-white object-cover" />
										<img src="/rondwell-attendee.png" alt="Attendee" class="h-5 w-5 rounded-full border border-white object-cover" />
									</div>
									<span class="text-sm text-gray-500">
										{ticket.registeredCount ?? 0} Registered
									</span>
								</div>
							</div>
						</div>
			...
			{:else}
				...
				<div class="flex flex-col rounded-md bg-[#FDFDFD] px-3">
					{#each tickets as ticket (ticket.id)}
						<div class="relative flex flex-col items-center justify-between gap-3 border-b py-3 last:border-b-0 lg:flex-row lg:gap-0">
							<div class="flex w-full items-center justify-between gap-2 lg:w-fit lg:justify-center">
								...
								<div class="flex w-full flex-col gap-3 lg:flex-row">
									<div class="flex items-center gap-3 lg:gap-2">
										...
										<span class="text-sm text-gray-500">{getTicketPriceLabel(ticket)}</span>
										<span class="rounded-full bg-[#F3F0FF] px-2 py-0.5 text-xs font-medium text-[#7C3AED]">
											{ticket.quantityAvailable ? `${ticket.quantityAvailable} Total` : 'Unlimited'}
										</span>
										...
										{#if ticket.tags?.length}
											{#each ticket.tags as tag}
												<span class="rounded-[11px] bg-[#E8F0FE] px-2 py-0.5 text-xs font-medium text-[#1A73E8]">{tag}</span>
											{/each}
										{/if}
									</div>
									<div class="mt-1 flex items-center gap-2 text-sm text-[#88888A]">
										<span class="flex items-center gap-1">
											<span class="h-2 w-2 rounded-full {getTicketStatusColor(ticket)}"></span>
							...
							<div class="flex w-full items-center justify-between gap-3 pl-7 lg:w-fit lg:justify-center lg:pl-0">
								<div class="flex items-center gap-1.5 text-sm text-[#A9AAAA]">
									<div class="flex -space-x-1.5">
										<img src="/rondwell-attendee.png" alt="Attendee" class="h-5 w-5 rounded-full border border-white object-cover" />
										<img src="/rondwell-attendee.png" alt="Attendee" class="h-5 w-5 rounded-full border border-white object-cover" />
									</div>
...
{#if showDeleteTicket}
	<DeleteTicket
		open={true}
		ticketName={tickets.find(t => t.id === showDeleteTicket)?.name ?? 'this ticket'}
		onDelete={() => handleDeleteTicket(showDeleteTicket ?? '')}
		onCancel={() => { showDeleteTicket = null; }}
	/>
{/if}
