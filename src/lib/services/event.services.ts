...
export interface CreateEventPayload {
	...
	locationDetails?: {
		...
		physical?: {
			venueName: string;
			venueAddress: string;
			resolvedAddress: { lat: number; lng: number; formatted_address: string };
		};
	};
	maxAttendees?: number;
	...
	/**
	 * Master ON/OFF for guest cash contributions at RSVP. The server treats
	 * effective enablement as `donationsEnabled && guestContributions.enabled`
	 * — never test either flag alone.
	 */
	donationsEnabled?: boolean;
	/**
	 * The server field is `publicAttendeeListEnabled`. `publicGuestListEnabled`
	 * is the name the create-event UI uses; both are accepted here and the
	 * canonical one is what gets sent.
	 */
	...
	// ── Celebration layer ────────────────────────────────────────────────
	// One named sub-document per feature, matching the convention the event
	// model already uses (`checkinSettings`, `pageSettings`, `agendaSettings`).
	// Every one of these is normalised + clamped server-side; the values sent
	// from here are a request, not an authority.

	/**
	 * Age gate. The age is computed against the event's START, not the
	 * registration date, so a guest who turns 18 the day before is eligible.
	 * `ID_REQUIRED` additionally demands approved KYC.
	 */
	ageRestriction?: {
		enabled: boolean;
		minimumAge?: number;
		verificationMethod?: 'SELF_DECLARED' | 'ID_REQUIRED';
	};
	/** Config for guest cash contributions. Master switch is `donationsEnabled`. */
	guestContributions?: {
		enabled?: boolean;
		suggestedAmountsKobo?: number[];
		allowCustom?: boolean;
		minKobo?: number;
		/** Omit or 0 for uncapped. */
		maxKobo?: number;
		currency?: 'NGN' | 'USD';
		message?: string;
		showOnPublicPage?: boolean;
	};
	/** Gift registry / wishlist pointer. The registry itself lives in products svc. */
	giftRegistry?: {
		enabled?: boolean;
		wishlistId?: string;
		showOnPublicPage?: boolean;
	};
	/** Guest photo uploads after the event, with a moderation queue. */
	memories?: {
		enabled?: boolean;
		visibility?: 'ATTENDEES_ONLY' | 'PUBLIC';
		requireApproval?: boolean;
		allowedFrom?: 'ANY_TIME' | 'AFTER_CHECKIN' | 'AFTER_EVENT_START';
		maxUploadsPerAttendee?: number;
		closesAt?: string | null;
		allowGuestDownload?: boolean;
	};
	/** Event budget tracking + vendor-invoice auto-reconciliation. */
	budget?: {
		enabled?: boolean;
	...
	/** Affiliate / promoter mode. `defaultCommissionPercent` is basis points. */
	promoter?: {
		enabled?: boolean;
...
export async function createEvent(payload: CreateEventPayload): Promise<CreateEventResponse> {
	// The create UI calls this toggle `publicGuestListEnabled`; the event model
	// calls it `publicAttendeeListEnabled`. Sending only the former meant the
	// choice was silently dropped by Mongoose. Normalise once, here, so every
	// caller (create flow, AI flow, clone) gets it right.
	const body: Record<string, any> = { ...payload };
	if ('publicGuestListEnabled' in body) {
		body.publicAttendeeListEnabled =
			body.publicAttendeeListEnabled ?? body.publicGuestListEnabled;
		delete body.publicGuestListEnabled;
	}
...
/**
 * Celebration-layer feature flags, RESOLVED server-side.
 *
 * Read these, not the raw sub-documents on `event` — `guestContributions.enabled`
 * here is already the AND of the legacy `donationsEnabled` master switch and
 * the config block, and `giftRegistry.enabled` already accounts for
 * `showOnPublicPage`. Re-deriving either on the client is how a public page
 * ends up offering to take money for an event whose owner turned it off.
 */
export interface PublicEventFeatures {
	guestContributions: {
		enabled: boolean;
		suggestedAmountsKobo: number[];
		allowCustom: boolean;
	...
	giftRegistry: { enabled: boolean; wishlistId: string | null };
	memories: {
		enabled: boolean;
		visibility: 'ATTENDEES_ONLY' | 'PUBLIC';
		allowedFrom: 'ANY_TIME' | 'AFTER_CHECKIN' | 'AFTER_EVENT_START';
...
export async function getPublicAgenda(
	eventId: string
): Promise<{ dates: any[]; rooms: any[]; eventDays: any[] }> {
...
export async function getPublicParticipants(
	eventId: string
): Promise<{ speakers: any[]; exhibitors: any[]; vendors: any[] }> {
...
// H-50 — `publishEvent()` REMOVED (2026-08-17). Draft is out: an event is LIVE
// from creation, so there is no publish step. The endpoint it called,
// `PUT /api/v1/events/:eventId/publish`, is deleted; it had never succeeded
// (`eventStatus` defaults to LIVE, so the server answered "Event is already
// live" for every event). No component called this — it was dead on both sides.

export async function uploadEventPhoto(
	eventId: string,
	file: File,
	category: 'COVER' | 'DISPLAY' = 'DISPLAY'
): Promise<string> {
...
export async function getEventAttendeesPaginated(
	...
	const res = await authFetch(
		`${EVENT_URL}/api/v1/events/${eventId}/attendees/paginated?${params.toString()}`
	);
...
export async function updateGroupRegistration(
	eventId: string,
	groupRegistrationEnabled: boolean,
	maxGroupSize?: number
): Promise<any> {
...
export async function getTicketRegistrationCount(
	eventId: string,
	ticketTypeId: string
): Promise<number> {
	try {
		const res = await authFetch(
			`${EVENT_URL}/api/v1/events/${eventId}/attendees/paginated?limit=1&ticketTypeId=${ticketTypeId}`
		);
		const data = await res.json();
		if (!res.ok) return 0;
...
export async function updateRegistrationField(
	...
	const res = await authFetch(
		`${EVENT_URL}/api/v1/events/${eventId}/registration-fields/${fieldId}`,
		{
			method: 'PUT',
			headers: { 'Content-Type': 'application/json' },
			body: JSON.stringify(payload)
...
export async function publishSeatLayout(eventId: string, layoutId: string): Promise<any> {
	const res = await authFetch(
		`${EVENT_URL}/api/v1/events/${eventId}/seat-layouts/${layoutId}/publish`,
		{
			method: 'PATCH'
		}
	);
...
export async function getEventSpeakers(
	eventId: string,
	filters?: { status?: string; search?: string; hasSession?: string }
): Promise<any[]> {
	const params = new URLSearchParams();
	params.set('role', 'SPEAKER');
	if (filters?.status && filters.status !== 'All')
		params.set('status', filters.status.toUpperCase().replace(' ', '_'));
	if (filters?.search) params.set('search', filters.search);

	const res = await authFetch(
		`${EVENT_URL}/api/v1/events/${eventId}/participants?${params.toString()}`
	);
...
export async function inviteSpeakerByEmail(
	eventId: string,
	payload: {
		firstName: string;
		lastName: string;
...
export async function inviteSpeakerByProfile(
	eventId: string,
	payload: {
...
export async function manualAddSpeaker(
	eventId: string,
	payload: {
		firstName: string;
		lastName: string;
...
export async function updateSpeakerDetails(
	...
	const res = await authFetch(
		`${EVENT_URL}/api/v1/events/${eventId}/participants/${participantId}/details`,
		{
			method: 'PUT',
			headers: { 'Content-Type': 'application/json' },
			body: JSON.stringify(payload)
...
export async function sendSpeakerMessage(
	eventId: string,
	participantId: string,
	payload: {
		subject: string;
	...
	const res = await authFetch(
		`${EVENT_URL}/api/v1/events/${eventId}/participants/${participantId}/send-message`,
		{
			method: 'POST',
			headers: { 'Content-Type': 'application/json' },
			body: JSON.stringify(payload)
...
export async function sendSpeakerReminder(
	eventId: string,
	participantId: string,
	payload: {
		reminderType: string;
	...
	const res = await authFetch(
		`${EVENT_URL}/api/v1/events/${eventId}/participants/${participantId}/send-reminder`,
		{
			method: 'POST',
			headers: { 'Content-Type': 'application/json' },
			body: JSON.stringify(payload)
...
export async function resendSpeakerInvitation(
	...
	const res = await authFetch(
		`${EVENT_URL}/api/v1/events/${eventId}/participants/${participantId}/resend-invitation`,
		{
			method: 'POST'
		}
	);
...
export async function removeSpeaker(eventId: string, participantId: string): Promise<void> {
	const res = await authFetch(
		`${EVENT_URL}/api/v1/events/${eventId}/participants/${participantId}`,
		{
			method: 'DELETE'
		}
	);
...
export async function assignSpeakerSessions(
	eventId: string,
	participantId: string,
	sessionIds: string[]
): Promise<any> {
	const res = await authFetch(
		`${EVENT_URL}/api/v1/events/${eventId}/participants/${participantId}/assign-sessions`,
		{
			method: 'PUT',
			headers: { 'Content-Type': 'application/json' },
...
export async function searchRondwellProfiles(
	...
	const res = await authFetch(
		`${EVENT_URL}/api/v1/events/${eventId}/participants/search-profiles?${params.toString()}`
	);
	const data = await res.json();
	if (!res.ok) return [];
...
export async function getEventExhibitors(
	eventId: string,
	filters?: { status?: string; search?: string; boothStatus?: string }
): Promise<any[]> {
	const params = new URLSearchParams();
...
export async function inviteExhibitorByEmail(
	eventId: string,
	payload: {
		firstName: string;
		lastName: string;
...
export async function inviteExhibitorByProfile(
	eventId: string,
	payload: {
		participantProfileId: string;
		participantUserId: string;
...
export async function manualAddExhibitor(
	eventId: string,
	payload: {
		firstName: string;
		lastName: string;
...
export async function sendExhibitorMessage(
	eventId: string,
	participantId: string,
	payload: {
		subject: string;
...
export async function sendExhibitorReminder(
	eventId: string,
	participantId: string,
	payload: {
		reminderType: string;
...
export async function resendExhibitorInvitation(
	eventId: string,
	participantId: string
): Promise<any> {
...
export async function approveParticipant(
	...
	const res = await authFetch(
		`${EVENT_URL}/api/v1/events/${eventId}/participants/${participantId}/approve`,
		{
			method: 'POST',
			headers: { 'Content-Type': 'application/json' },
			body: JSON.stringify(payload)
...
export async function declineParticipant(
	...
	const res = await authFetch(
		`${EVENT_URL}/api/v1/events/${eventId}/participants/${participantId}/decline`,
		{
			method: 'POST',
			headers: { 'Content-Type': 'application/json' },
			body: JSON.stringify(payload)
...
export async function sendPaymentRequest(eventId: string, participantId: string): Promise<any> {
	const res = await authFetch(
		`${EVENT_URL}/api/v1/events/${eventId}/participants/${participantId}/send-payment-request`,
		{
			method: 'POST'
		}
	);
...
export async function markPaidOffline(
	...
	const res = await authFetch(
		`${EVENT_URL}/api/v1/events/${eventId}/participants/${participantId}/mark-paid`,
		{
			method: 'POST',
			headers: { 'Content-Type': 'application/json' },
			body: JSON.stringify(payload || {})
...
export async function getEventVendors(
	eventId: string,
	filters?: { status?: string; search?: string; hasOrders?: string }
): Promise<any[]> {
	const params = new URLSearchParams();
...
export async function inviteVendorByProfile(
	eventId: string,
	payload: {
		participantProfileId: string;
		participantUserId: string;
...
export async function manualAddVendor(
	eventId: string,
	payload: {
		firstName: string;
		lastName: string;
...
export async function getCollaborationRequests(
	...
	const res = await authFetch(
		`${EVENT_URL}/api/v1/events/${eventId}/participants/collaboration-requests?${params.toString()}`
	);
...
export async function respondToCollaborationRequest(
	...
	const res = await authFetch(
		`${EVENT_URL}/api/v1/events/${eventId}/participants/collaboration-requests/${requestId}/respond`,
		{
			method: 'POST',
			headers: { 'Content-Type': 'application/json' },
...
export async function uploadRoomBanner(
	eventId: string,
	file: File
...
export async function updateEventSession(
	...
	const res = await authFetch(
		`${EVENT_URL}/api/v1/events/${eventId}/rooms/${roomId}/sessions/${sessionId}`,
		{
			method: 'PUT',
			headers: { 'Content-Type': 'application/json' },
			body: JSON.stringify(payload)
...
export async function deleteEventMedia(eventId: string, mediaId: string): Promise<void> {
	const res = await authFetch(`${EVENT_URL}/api/v1/events/${eventId}/media/${mediaId}`, {
		method: 'DELETE'
	});
...
export async function uploadEventMedia(
	...
	formData.append(
		'type',
		file.type.startsWith('video') ? 'VIDEO' : file.type === 'application/pdf' ? 'DOCUMENT' : 'IMAGE'
	);
...
/**
 * Planning checklists for an event.
 *
 * The backend has exposed `/events/:id/checklists` since the checklist model
 * landed, but no frontend caller existed — so checklist data never appeared
 * anywhere in the product, including the event report. Read access only needs
 * accepted event access; mutations need CHECKLIST_MANAGE.
 */
export async function getEventChecklists(eventId: string): Promise<any[]> {
	const res = await authFetch(`${EVENT_URL}/api/v1/events/${eventId}/checklists`);
	if (!res.ok) await throwApiError(res, 'Failed to fetch checklists');
	const data = await res.json();
	return data.checklists ?? data.data ?? data ?? [];
}

export async function createEventFaq(
	eventId: string,
	payload: { question: string; answer: string; category?: string; isPublic?: boolean }
): Promise<any> {
	const res = await authFetch(`${EVENT_URL}/api/v1/events/${eventId}/faqs`, {
		method: 'POST',
		headers: { 'Content-Type': 'application/json' },
...
export async function inviteEventAdmin(
	eventId: string,
	payload: InviteEventAdminPayload
): Promise<any> {
...
export async function updateEventAdminPermissions(
	eventId: string,
	adminId: string,
	payload: { role?: string; permissions?: string[]; showOnEventPage?: boolean }
): Promise<any> {
	const res = await authFetch(
		`${EVENT_URL}/api/v1/events/${eventId}/admin/${adminId}/permissions`,
		{
			method: 'PUT',
			headers: { 'Content-Type': 'application/json' },
			body: JSON.stringify(payload)
...
export async function removeEventAdmin(eventId: string, adminId: string): Promise<void> {
	const res = await authFetch(`${EVENT_URL}/api/v1/events/${eventId}/admin/${adminId}`, {
		method: 'DELETE'
	});
...
export async function resendAdminInvite(eventId: string, adminId: string): Promise<any> {
	const res = await authFetch(
		`${EVENT_URL}/api/v1/events/${eventId}/admin/${adminId}/resend-invite`,
		{ method: 'POST' }
	);
	if (!res.ok) await throwApiError(res, 'Failed to resend invitation');
	const data = await res.json();
...
export async function getAdminInvitation(eventId: string, token: string): Promise<any> {
	const res = await fetch(
		`${EVENT_URL}/api/v1/events/${eventId}/admin/invitation/${encodeURIComponent(token)}`
	);
	if (!res.ok) {
...
export async function getAdminRolesCatalog(): Promise<
	Array<{
		value: string;
		label: string;
		description: string;
...
export async function getAttendeeDetail(
	...
	const res = await authFetch(
		`${EVENT_URL}/api/v1/events/${eventId}/attendees/${attendeeId}/detail?_t=${Date.now()}`
	);
...
export async function getAttendeeTimeline(eventId: string, attendeeId: string): Promise<any[]> {
	const res = await authFetch(
		`${EVENT_URL}/api/v1/events/${eventId}/attendees/${attendeeId}/timeline?_t=${Date.now()}`
	);
...
export async function updateAttendeeStatus(
	...
	const res = await authFetch(
		`${EVENT_URL}/api/v1/events/${eventId}/attendees/${attendeeId}/status`,
		{
			method: 'PATCH',
			headers: { 'Content-Type': 'application/json' },
...
export async function reportAttendee(
	eventId: string,
	attendeeId: string,
	notes: string,
	blockForFutureEvents: boolean
): Promise<any> {
	const res = await authFetch(
		`${EVENT_URL}/api/v1/events/${eventId}/attendees/${attendeeId}/report`,
		{
			method: 'POST',
			headers: { 'Content-Type': 'application/json' },
...
export async function addAttendeeTag(
	eventId: string,
	attendeeId: string,
	tagName: string
): Promise<any> {
	const res = await authFetch(
		`${EVENT_URL}/api/v1/events/${eventId}/attendees/${attendeeId}/tags`,
		{
			method: 'POST',
			headers: { 'Content-Type': 'application/json' },
...
export async function removeAttendeeTag(
	eventId: string,
	attendeeId: string,
	tagName: string
): Promise<any> {
	const res = await authFetch(
		`${EVENT_URL}/api/v1/events/${eventId}/attendees/${attendeeId}/tags/${encodeURIComponent(tagName)}`,
		{
			method: 'DELETE'
		}
	);
...
export async function transferEventCollection(
	eventId: string,
...
export async function getCollectionEvents(
	...
	const res = await authFetch(
		`${EVENT_URL}/api/v1/collections/${collectionId}/events?filter=${filter}`
	);
	const data = await res.json();
	if (!res.ok) return [];
	return data.events ?? [];
...
export async function addExistingEventToCollection(
	collectionId: string,
	eventId: string
): Promise<any> {
	const res = await authFetch(
		`${EVENT_URL}/api/v1/collections/${collectionId}/events/add-existing`,
		{
			method: 'PUT',
			headers: { 'Content-Type': 'application/json' },
...
export async function resolveEventUrl(collectionId: string, url: string): Promise<any> {
	const res = await authFetch(
		`${EVENT_URL}/api/v1/collections/${collectionId}/events/resolve-url`,
		{
			method: 'POST',
			headers: { 'Content-Type': 'application/json' },
...
export async function addExternalEventToCollection(
	collectionId: string,
	payload: {
		title: string;
		description?: string;
...
export async function uploadCollectionProfilePicture(
	collectionId: string,
	file: File
...
export async function uploadCollectionCoverBanner(
	collectionId: string,
	file: File
): Promise<{ coverBannerUrl: string }> {
	const form = new FormData();
	form.append('file', file);
...
export async function requestCollectionVerification(
	collectionId: string,
	payload: {
	...
	const res = await authFetch(
		`${EVENT_URL}/api/v1/collections/${collectionId}/request-verification`,
		{
			method: 'POST',
			headers: { 'Content-Type': 'application/json' },
			body: JSON.stringify(payload)
...
export async function getCollectionVerificationStatus(
	collectionId: string
): Promise<{ approvalStatus: string; verificationRequest?: any }> {
	const res = await authFetch(
		`${EVENT_URL}/api/v1/collections/${collectionId}/verification-status`
	);
...
export async function createEventBlast(
	eventId: string,
	payload: {
		subject: string;
		htmlContent: string;
...
export async function getEventBlasts(
	eventId: string,
...
export async function cancelEventBlast(eventId: string, blastId: string): Promise<any> {
	const res = await authFetch(`${EVENT_URL}/api/v1/events/${eventId}/blasts/${blastId}/cancel`, {
		method: 'POST'
	});
...
export async function sendBlastNow(eventId: string, blastId: string): Promise<any> {
	const res = await authFetch(`${EVENT_URL}/api/v1/events/${eventId}/blasts/${blastId}/send-now`, {
		method: 'POST'
	});
...
export async function previewEventBlast(eventId: string, blastId: string): Promise<any> {
	const res = await authFetch(`${EVENT_URL}/api/v1/events/${eventId}/blasts/${blastId}/preview`, {
		method: 'POST'
	});
...
export async function previewBlastDraft(
	eventId: string,
	subject: string,
	htmlContent: string
): Promise<any> {
 ...
 */
export async function uploadEventDescriptionImage(file: File, eventId?: string): Promise<string> {
	const formData = new FormData();
	formData.append('image', file);
	const url = eventId
...
export async function createCollectionBlast(
	collectionId: string,
	payload: {
		subject: string;
		htmlContent: string;
		recipientFilters?: { source?: string[] };
		scheduledAt?: string;
	}
): Promise<any> {
...
export async function getCollectionBlasts(
	collectionId: string,
	params: Record<string, string> = {}
): Promise<any> {
	const query = new URLSearchParams(params).toString();
...
export async function cancelCollectionBlast(collectionId: string, blastId: string): Promise<any> {
	const res = await authFetch(
		`${EVENT_URL}/api/v1/collections/${collectionId}/blasts/${blastId}/cancel`,
		{ method: 'POST' }
	);
	if (!res.ok) await throwApiError(res, 'Failed to cancel blast');
	const data = await res.json();
...
export async function sendCollectionBlastNow(collectionId: string, blastId: string): Promise<any> {
	const res = await authFetch(
		`${EVENT_URL}/api/v1/collections/${collectionId}/blasts/${blastId}/send-now`,
		{ method: 'POST' }
	);
	if (!res.ok) await throwApiError(res, 'Failed to send blast');
	const data = await res.json();
...
export async function previewCollectionBlastDraft(
	collectionId: string,
	subject: string,
	htmlContent: string
): Promise<any> {
	const res = await authFetch(
		`${EVENT_URL}/api/v1/collections/${collectionId}/blasts/preview-draft`,
		{
			method: 'POST',
			headers: { 'Content-Type': 'application/json' },
			body: JSON.stringify({ subject, htmlContent })
...
export async function uploadCollectionBlastImage(
	collectionId: string,
	file: File
): Promise<string> {
	const formData = new FormData();
...
// ==================== CHECK-IN APIs ====================
//
// C-22 — All four of these used plain `fetch` with no `Authorization` header,
// which is what the audit points at as the frontend already knowing the
// endpoints were unauthenticated. The backend now requires `authMiddleware` +
// `requireEventAccess([CHECKIN_MANAGE])` on every one of them, so these must go
// through `authFetch` (which also handles the 401 refresh-and-retry).
...
export async function verifyCheckinPasscode(eventId: string, passcode: string): Promise<any> {
	const res = await authFetch(`${EVENT_URL}/api/v1/checkin/verify/passcode`, {
		method: 'POST',
		headers: { 'Content-Type': 'application/json' },
...
export async function verifyCheckinQr(eventId: string, checkinToken: string): Promise<any> {
	const res = await authFetch(`${EVENT_URL}/api/v1/checkin/verify/qr`, {
		method: 'POST',
		headers: { 'Content-Type': 'application/json' },
...
export async function checkinByPasscode(eventId: string, passcode: string): Promise<any> {
	const res = await authFetch(`${EVENT_URL}/api/v1/checkin/passcode`, {
		method: 'POST',
		headers: { 'Content-Type': 'application/json' },
		body: JSON.stringify({ eventId, passcode })
	});
...
export async function checkinByQrCode(eventId: string, checkinToken: string): Promise<any> {
	const res = await authFetch(`${EVENT_URL}/api/v1/checkin/qr-scan`, {
		method: 'POST',
		headers: { 'Content-Type': 'application/json' },
		body: JSON.stringify({ eventId, checkinToken })
	});
...
export async function updateEventDay(
	dayId: string,
	payload: Partial<EventDayPayload>
): Promise<any> {
...
export async function bulkReplaceEventDays(
	eventId: string,
