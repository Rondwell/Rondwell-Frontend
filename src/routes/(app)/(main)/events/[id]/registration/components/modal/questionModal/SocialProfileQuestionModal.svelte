<!-- src/lib/components/SocialProfileQuestionModal.svelte -->
<script lang="ts">
	...
	const socialPlatforms = ['Instagram', 'LinkedIn', 'X (Twitter)', 'Youtube', 'Facebook', 'TikTok'];

	function selectPlatform(platform: string) {
		questionData.platform = platform;
		updateQuestionText(platform);
	}
	...
	function updatePlatform(e: any) {
		questionData.platform = e.target.value;
		// Update question text based on selected platform
		updateQuestionText(questionData.platform);
	}
	...
	// Update question text based on selected platform
	function updateQuestionText(platform: string) {
		const platformLabel = platform;
		if (
			!questionData.question ||
			questionData.question.includes('username') ||
			questionData.question.includes('link')
		) {
			questionData.question = `What is your ${platformLabel} username?`;
		}
	}
...
{#if open}
	<div class="fixed inset-0 z-50 flex items-center justify-center px-3 bg-black/50 backdrop-blur-sm">
		<div
		...
		>
			...
			<div class="space-y-6">
				<div class="flex items-center space-x-3">
					...
					>
						<img src="/user-octagon.svg" alt="arrow back" />
					</div>
					<div>
						<h3 class="font-medium">Social Profile</h3>
						<p class="text-sm text-gray-600">Ask for a social network username or link</p>
					</div>
				</div>

				<div class="space-y-4">
					<div>
						<label for="platform" class="mb-1 block text-sm font-medium text-[#696B6D]"
							>Platform</label
						>
						<div use:clickOutside={() => (showPlatformDropdown = false)} class="relative w-full">
							<button
								...
								on:click={() => (showPlatformDropdown = !showPlatformDropdown)}
							>
								<span class="">
								...
								>
									<div class="flex flex-col">
										{#each socialPlatforms as platform}
											...
											{questionData.platform === platform ? 'bg-[#F0F0F0]' : ''}"
											>
												<p>{platform}</p>

												{#if questionData.platform === platform}
					...
					<div>
						...
						/>
						<div class="mt-2 text-xs text-gray-400">{questionData.helpText}</div>
					</div>
				</div>
